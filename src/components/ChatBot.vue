...
async function sendUserMessage() {
  ...
  const aiMessage = await sendMessageToAIService(currentUserMessage.message)

  const currentAIMessage = { type: 'ai', message: aiMessage }
...
<template>
  <div class="fixed right-4 bottom-4 z-40">
    <button
      class="fixed bottom-4 right-4 inline-flex items-center justify-center text-sm font-medium disabled:pointer-events-none disabled:opacity-50 border rounded-full w-16 h-16 bg-black hover:bg-gray-700 m-0 cursor-pointer border-gray-200 bg-none p-0 normal-case leading-5 hover:text-gray-900 z-30"
      type="button"
      @click="handleToggleChatBox"
    ...
    >
      ...
      >
        ...
        <div v-for="message in allMessages" :key="message.message">
          <!--  User Chat Message -->
          <div v-if="message.type === 'user'" class="flex gap-3 my-4 text-gray-600 text-sm flex-1">
            ...
            <p class="leading-relaxed">
              <span class="block font-bold text-gray-700">You </span>{{ message.message }}
            </p>
          </div>
          <!-- Ai Chat Message  -->
          <div v-if="message.type === 'ai'" class="flex gap-3 my-4 text-gray-600 text-sm flex-1">
            <span class="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8">
              <div class="rounded-full bg-gray-100 border p-1">
                <svg
                  stroke="none"
          ...
          <div class="leading-relaxed">
            ...
            <div>
              <div class="flex space-x-2 justify-center items-center mt-4">
                ...
                <div
                  class="h-2 w-2 bg-slate-700 rounded-full animate-bounce [animation-delay:-0.3s]"
                ></div>
                <div
                  class="h-2 w-2 bg-slate-700 rounded-full animate-bounce [animation-delay:-0.15s]"
                ></div>
                <div class="h-2 w-2 bg-slate-700 rounded-full animate-bounce"></div>
              </div>
            </div>
          </div>
        </div>
